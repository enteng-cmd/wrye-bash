    ...
    def updateRecords(self, srcBlock, mapper, mergeIds):
        ...
        for record in srcBlock.getActiveRecords():
            ...
            if src_dial_fid in id_dials:
                ...
                # We can only discard the DIAL record from mergeIds if none of
                # its children are marked as merged, since otherwise we'd end
                # up wanting to keep a child but not its parent
        ...
        for record in block.getActiveRecords():
            ...
            for info_rec in record.infos:
                ...
                if doFilter:
                    ...
                    masterset = MasterSet()
                    info_rec.updateMasters(masterset.add)
                    if not loadSetIsSuperset(masterset):
                        continue
